...
internal sealed class FastResourceLock : IDisposable
{
    ...
    private static readonly int m_SpinCount = NativeMethods.SpinCount;

    private int m_Value;
    private IntPtr m_SharedWakeEvent;
    private IntPtr m_ExclusiveWakeEvent;
    ...
    public FastResourceLock()
    {
        ...
        m_SharedWakeEvent = NativeMethods.CreateSemaphore(IntPtr.Zero, 0, int.MaxValue, null);
        m_ExclusiveWakeEvent = NativeMethods.CreateSemaphore(IntPtr.Zero, 0, int.MaxValue, null);
#endif
    }
    ...
    {
        if (m_SharedWakeEvent != IntPtr.Zero)
        {
            NativeMethods.CloseHandle(m_SharedWakeEvent);
            m_SharedWakeEvent = IntPtr.Zero;
        }

        if (m_ExclusiveWakeEvent != IntPtr.Zero)
        {
            NativeMethods.CloseHandle(m_ExclusiveWakeEvent);
            m_ExclusiveWakeEvent = IntPtr.Zero;
        }
    }
    ...
    public int ExclusiveWaiters
    {
        get
        {
            return (m_Value >> LockExclusiveWaitersShift) & LockExclusiveWaitersMask;
        }
    }
    ...
    public int SharedOwners
    {
        get
        {
            return (m_Value >> LockSharedOwnersShift) & LockSharedOwnersMask;
        }
    }
    ...
    public int SharedWaiters
    {
        get
        {
            return (m_Value >> LockSharedWaitersShift) & LockSharedWaitersMask;
        }
    }
    ...
    public void ReleaseExclusive()
    {
        ...
        while (true)
        {
            ...
            if (((value >> LockExclusiveWaitersShift) & LockExclusiveWaitersMask) != 0)
            {
                ...
                {
                    NativeMethods.ReleaseSemaphore(m_ExclusiveWakeEvent, 1, IntPtr.Zero);
    ...
    public void ReleaseShared()
    {
        ...
        while (true)
        {
            ...
            else if (((value >> LockExclusiveWaitersShift) & LockExclusiveWaitersMask) != 0)
            {
                if (Interlocked.CompareExchange(
                        ref m_Value,
                        value - LockOwned + LockExclusiveWaking - LockSharedOwnersIncrement -
                        LockExclusiveWaitersIncrement,
                        value
                    ) == value)
                {
                    NativeMethods.ReleaseSemaphore(m_ExclusiveWakeEvent, 1, IntPtr.Zero);
